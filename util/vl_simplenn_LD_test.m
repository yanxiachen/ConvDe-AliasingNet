function res = vl_simplenn_LD_test(net, data)
...
%         if opts.conserveMemory && ~net.layerin{j}.precious && j ~= Ni
%             resin(j+1).dzdx = [] ;
%             resin(j+1).x = [] ;
%         end
        if gpuMode && opts.sync
...
%         resin(j).backwardTime = toc(resin(j).backwardTime) ;
    end
    if n > 1 && n == backPropLim && opts.conserveMemory && ~net.layers{n}.precious
        res(n).dzdx = [] ;
        res(n).x = [] ;
    end
%     if j > 1 && j == backPropLim && opts.conserveMemory && ~net.layerin{j}.precious
%         resin(j).dzdx = [] ;
%         resin(j).x = [] ;
%     end
end
